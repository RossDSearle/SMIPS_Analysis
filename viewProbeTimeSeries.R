...
for (i in 1:nrow(smlocs)) {
 ...
 c<-ggplot(b, aes(x=date,y=Moisture, group=Depth))+
   ...
   geom_line(aes(y = smipsDF$v * 100, x=smipsDF$t), color = "black", linetype = "dotted") +
   theme_light()+
 ...
 row<-i
 filename<-paste0("c:/temp/P/Html_PlotlyTest/",row,"_",ID,".html")
 htmlwidgets::saveWidget(as_widget(fig), filename[1],selfcontained = FALSE)
 
 #Export plots
 filename2<-paste0("c:/temp/P/PlotsTest/",row,"_",ID,".png")
 #tiff(filename = filename2[1],width=1500, height=750,units="px",res=100)
 png(filename = filename2[1],width=1500, height=750,units="px",res=100)
